...
export default function collectorFWorkflow({wall, input}) {
    ...
    if (!hasWallGap){
        ...
        let totShear = calculateTotalShear(w, L);
        let diaphragmShear = calculateUnitShear(totShear, d);
        let R = calculateRx(L, w);
        let vWall = calculateVWall(R,wallDistanceCovered)
